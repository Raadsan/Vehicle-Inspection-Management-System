...
export default function RolesPage() {
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try { await roleApi.create({ name, description: description || undefined }); toast.success("Role created"); setIsAddOpen(false); loadData() }
    catch (err: any) { toast.error("Failed: " + (err.response?.data?.error || err.message)) }
  ...
  const handleEdit = async (e: React.FormEvent) => {
    ...
    try { await roleApi.update(selected.id, { name, description: description || undefined }); toast.success("Role updated"); setIsEditOpen(false); loadData() }
    catch (err: any) { toast.error("Failed: " + (err.response?.data?.error || err.message)) }
    finally { setSubmitting(false) }
  }
  ...
  const handleDelete = async () => {
    ...
    try { await roleApi.delete(selected.id); toast.success("Role deleted"); setIsDeleteOpen(false); loadData() }
    catch (err: any) { toast.error("Failed: " + (err.response?.data?.error || err.message)) }
    finally { setSubmitting(false) }
  }

  const openAdd = () => { setName(""); setDescription(""); setIsAddOpen(true) }
  const openEdit = (r: Role) => { setSelected(r); setName(r.name); setDescription(r.description || ""); setIsEditOpen(true) }
  ...
  return (
    <div className={cn(dashboardPageClass, "space-y-5")} style={dashboardPageStyle}>
      <div className={pageHeaderWrapperClass}>
        ...
        <p className={pageHeaderSubtitleClass}>Manage custom permission roles</p>
      </div>
      ...
      <div className={dashboardCardClass}>
        <div className="px-5 py-3.5 flex flex-wrap items-center justify-between gap-3 border-b border-zinc-100 dark:border-border">
          <span className="text-sm font-semibold text-zinc-500">{filtered.length} of {roles.length} roles</span>
          <div className="flex items-center gap-2">
            ...
            <Button onClick={openAdd} className="h-10 bg-[#1565c0] hover:bg-[#0a2744] text-white font-semibold px-5 rounded-md flex items-center gap-2 shadow-sm text-sm">
              <Plus className="size-4.5" /><span>Add Role</span>
            </Button>
          </div>
        </div>
            ...
            <TableBody className="bg-card">
              {loading ? (
                ...
                <TableRow key={row.id} className={dashboardTableBodyRowClass}>
                  ...
                  <TableCell className={dashboardTableCellClass}><span className="text-sm font-semibold text-foreground flex items-center gap-1.5"><Shield className="size-4 text-[#1565c0] shrink-0" />{row.name}</span></TableCell>
                  <TableCell className={dashboardTableCellClass}><span className="text-sm text-zinc-500 font-normal">{row.description || "—"}</span></TableCell>
                ...
                <TableRow><TableCell colSpan={4} className="py-12 text-center text-zinc-400 text-sm">No roles found</TableCell></TableRow>
              )}
            </TableBody>
          </Table>
        </div>
      ...
      <Dialog open={isAddOpen} onOpenChange={setIsAddOpen}>
        <DialogContent className="max-w-3xl bg-white dark:bg-card border border-border rounded-lg p-8">
          <DialogHeader className="mb-4">
            ...
            <DialogDescription className="text-sm text-zinc-400 font-normal">Define a custom permission role.</DialogDescription>
          </DialogHeader>
          <form onSubmit={handleAdd} className="grid grid-cols-2 gap-4 py-2">
            <div className="col-span-2"><label className={labelCls}>Role Name *</label><input required type="text" placeholder="Role name" value={name} onChange={(e) => setName(e.target.value)} className={inputCls} /></div>
            <div className="col-span-2"><label className={labelCls}>Description</label>
              <textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Describe this role..."
                className="w-full h-24 px-3 py-2.5 border border-zinc-200 dark:border-border rounded-md outline-none text-sm bg-white dark:bg-muted/10 focus:border-[#1565c0] transition-all font-normal resize-none" />
            </div>
      ...
      <Dialog open={isEditOpen} onOpenChange={setIsEditOpen}>
        <DialogContent className="max-w-3xl bg-white dark:bg-card border border-border rounded-lg p-8">
          <DialogHeader className="mb-4">
            ...
            <DialogDescription className="text-sm text-zinc-400 font-normal">Update role name or description.</DialogDescription>
          </DialogHeader>
          <form onSubmit={handleEdit} className="grid grid-cols-2 gap-4 py-2">
            <div className="col-span-2"><label className={labelCls}>Role Name *</label><input required type="text" value={name} onChange={(e) => setName(e.target.value)} className={inputCls} /></div>
            <div className="col-span-2"><label className={labelCls}>Description</label>
      ...
      <Dialog open={isViewOpen} onOpenChange={setIsViewOpen}>
        <DialogContent className="max-w-lg bg-white dark:bg-card border border-border rounded-lg p-8">
          <DialogHeader className="mb-4"><DialogTitle className="text-lg font-bold text-[#0a2744] dark:text-white">Role Details</DialogTitle></DialogHeader>
          {selected && (
            <div className="space-y-3">
              {[["Name", selected.name], ["Description", selected.description || "—"]].map(([l, v]) => (
      ...
      <Dialog open={isDeleteOpen} onOpenChange={setIsDeleteOpen}>
        <DialogContent className="max-w-md bg-white dark:bg-card border border-border rounded-lg p-8">
          <DialogHeader className="mb-4">
            ...
            <DialogDescription className="text-sm text-zinc-500 font-normal">This will permanently remove this role.</DialogDescription>
          </DialogHeader>
