...
export default function RejectedInspectionsPage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      setInspections(all.filter((i: Inspection) => i.status === "REJECTED"))
    } catch (err: unknown) {
    ...
    <div className={cn(dashboardPageClass, "space-y-5")} style={dashboardPageStyle}>
      <div className={pageHeaderWrapperClass}>
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className={pageHeaderTitleClass}>Rejected Inspections</h1>
            <p className={pageHeaderSubtitleClass}>Inspections that were rejected during approval review</p>
          </div>
        </div>
      </div>
          ...
          <Table className="w-full">
            <TableHeader className={dashboardTableHeaderClass}>
              <TableRow className={dashboardTableHeadRowClass}>
                {["NO", "Vehicle", "Company", "Scheduled", "Result", "Status", "Actions"].map(h => (
            ...
            <TableBody className="bg-card">
              {loading ? (
                ...
                <TableRow key={row.id} className={dashboardTableBodyRowClass}>
                  ...
                  <TableCell className={dashboardTableCellClass}><span className="text-sm text-zinc-500">{row.scheduledAt ? new Date(row.scheduledAt).toLocaleDateString() : "—"}</span></TableCell>
                  <TableCell className={dashboardTableCellClass}>
                    {row.overallResult ? (
                      <span className={cn(dashboardStatusBadgeClass, row.overallResult === "PASS" ? "bg-emerald-600 text-white" : "bg-rose-600 text-white")}>{row.overallResult}</span>
                    ) : "—"}
                  </TableCell>
                  <TableCell className={dashboardTableCellClass}>
                ...
                <TableRow><TableCell colSpan={7} className="py-16 text-center text-zinc-400 text-sm">No rejected inspections</TableCell></TableRow>
              )}
            </TableBody>
          </Table>
        </div>
      ...
      <Dialog open={isViewOpen} onOpenChange={setIsViewOpen}>
        <DialogContent className="max-w-lg bg-white dark:bg-card border border-border rounded-lg p-8">
          <DialogHeader className="mb-4">
            <DialogTitle className="text-lg font-bold text-rose-600">Inspection #{selected?.id} — Rejected</DialogTitle>
          </DialogHeader>
          {selected && (
            <div className="space-y-3">
              {[["Vehicle", selected.vehicle?.plateNumber], ["Company", selected.company?.name], ["Scheduled", selected.scheduledAt ? new Date(selected.scheduledAt).toLocaleString() : "—"], ["Result", selected.overallResult || "—"], ["Notes", selected.notes || "—"]].map(([l, v]) => (
