...
import { Plus, Edit, Trash2, Loader2, Car, Search } from "lucide-react"
    ...
    <div className={cn(dashboardPageClass, "space-y-5")} style={dashboardPageStyle}>
      ...
      <div className={dashboardCardClass}>
        <div className="px-5 py-3.5 flex flex-wrap items-center justify-between gap-3 border-b border-zinc-100 dark:border-border">
          <span className="text-sm font-semibold text-zinc-500">{filtered.length} of {models.length} models</span>
          ...
          <Table className="w-full">
            <TableHeader className={dashboardTableHeaderClass}>
              <TableRow className={dashboardTableHeadRowClass}>
                {["#", "Model Name", "Brand", "Description", "Year", "Created By", "Actions"].map(h => (
            ...
            <TableBody className="bg-card">
              {loading ? (
                  ...
                  <TableRow key={model.id} className={dashboardTableBodyRowClass}>
                    ...
                    <TableCell className={dashboardTableCellClass}>
                      <span className="text-sm font-bold text-foreground flex items-center gap-2">
                        <Car className="size-4 text-[#1565c0] shrink-0" />
                        {model.name}
                      </span>
                    </TableCell>
                    <TableCell className={dashboardTableCellClass}>
                ...
                <TableRow>
                  <TableCell colSpan={7} className="py-12 text-center text-zinc-400 text-sm">No models found</TableCell>
                </TableRow>
              )}
            </TableBody>
          </Table>
